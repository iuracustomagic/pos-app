...
function UserModal({ arg }) {
  const { username, _id } = useMemo(() => parseJwt(localStorage.getItem('jwt')).data, []);
          ...
          <>
            ...
            <div className="modal-body">
              <div className="form-group">
                ...
                <input id="userName"  onChange={(e) => setSendData((prev) => ({ ...prev, username: e.target.value }))}
              ...
              {
                arg ? (
                  <Permissions getRights={getRights} rights={{ admin: sendData.admin, manager: sendData.manager }} />
                ) : null
              }
              <div className="d-flex mt-4">
                <button onClick={saveUser} className="btn btn-primary" type="button">{ translate[lang].Save }</button>
                <button onClick={() => dispatch(setDialog(['Confirm', { action: quit, description: 'You are about to log out.' }]))} type="button" className="btn btn-default waves-effect waves-light">
